...
export const LoginPage: React.FC<LoginPageProps> = ({ setActiveTab }) => {
  const { login, loginWithGoogle, connectDiscord } = useAuth();
  const { t } = useLanguage();
  ...
  const handleDiscordSignIn = async () => {
    ...
    try {
      ...
      setError(err.message || 'Failed to initialize Discord OAuth');
    } finally {
  ...
  return (
    <div className="py-12 px-4 max-w-md mx-auto">
      <div className="p-8 rounded-3xl bg-slate-900 border border-cyan-500/20 shadow-2xl space-y-6">
        ...
        <div className="p-3.5 rounded-2xl bg-slate-950 border border-cyan-500/20 space-y-2">
          <p className="text-[10px] uppercase font-extrabold tracking-wider text-cyan-300 text-center">
          ...
          <div className="grid grid-cols-2 gap-2">
            <button
              ...
              className="py-2 px-3 rounded-xl bg-cyan-500/10 hover:bg-cyan-500/20 text-cyan-300 text-xs font-bold border border-cyan-500/30 transition-colors flex items-center justify-center gap-1.5"
            >
        ...
        <div className="space-y-2.5">
          ...
          <button
            ...
            className="w-full py-2.5 px-4 rounded-xl font-semibold text-xs text-white bg-[#5865F2] hover:bg-[#4752C4] transition-all flex items-center justify-center gap-2.5 shadow-lg shadow-[#5865F2]/20 active:scale-[0.99]"
          >
            {discordLoading ? (
              <Loader2 className="w-4 h-4 animate-spin text-white" />
            ) : (
              <svg className="w-4 h-4 shrink-0 fill-current" viewBox="0 0 127.14 96.36">
            ...
            <span>Connect & Sign in with Discord</span>
          </button>
